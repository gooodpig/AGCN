...
普通独立点会生成 `dot()`。当某点同时位于两个不同的可见对象上时（包括直线、线段、圆、圆锥曲线和隐式曲线），交点本身通常已经足够清楚，因此不会额外生成实心点。
...
- GeoGebra `implicitpoly` 对象中的三次多项式系数会被读取、归一化，并作为零等值线绘制

三次曲线的系数按 `x` 次数和 `y` 次数恢复为隐式方程；整体缩放不会改变曲线，因此生成器会先归一化系数，减少数值过大造成的绘制误差。标签布局也会把三次曲线的局部切线纳入避让计算。
...
可见角对象会生成半径随画布缩放的小圆弧 `arc()`。当前版本支持三点定义的角，也支持两条直线、线段或坐标轴之间的交角。程序按几何图常用习惯绘制锐角标记，并自动调整方向，避免错误地标记成外角、钝角或过大的角弧。
...
- 青色、蓝色、红色等明显彩色对象保留颜色
- GeoGebra 默认蓝色点在自动模式中输出为黑色；其他明显彩色点仍保留颜色
...
- 一般圆锥曲线在该点处的切线
- 标签与直线、曲线和其他点的距离
- GeoGebra 中已有的手工标签偏移（作为偏好而非不可修改的硬约束）

布局器会先为所有标签建立 8 个方向和多档边距的候选边界框，再从多个初始布局出发进行全局迭代。距离模型按照 Asymptote 的实际行为计算：`1.5*N` 只会放大文字框与点之间的边距，不会将整个文字框按比例平移。评分同时计算标签框互撞、安全间距、线与曲线穿过标签、邻近点、画布溢出和 GeoGebra 手工偏移。普通情况优先使用 `1`、`1.25`、`1.5` 倍边距；`2`、`2.5`、`3`、`4` 倍仅在换方向仍无法消除几何冲突时使用。GeoGebra 手工偏移会提高对应方向的优先级，但不会阻止程序把压在线上的标签移开。
...
当 GeoGebra 显示坐标轴时，转换器会保留原视口的宽高比和完整轴线，并在四周加入少量留白，避免箭头和轴标签被裁掉。
...
当前标签布局还使用 76 个真实 GeoGebra 文件（70 个去重图形）进行批量回归：全部文件均可转换，70/70 个唯一图形均可由 Asymptote 编译；在 625 个可见标签的估算边界框中未发现标签互相重叠；估算的可见直线穿过标签框情况已降至 29 个，剩余情况主要集中在同时经过多条直线的极端密集构型。
