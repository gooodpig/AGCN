...
布局器会先为所有标签建立 8 个方向和多档距离的候选边界框，再从多个初始布局出发进行全局迭代。评分同时计算标签框互撞、安全间距、线与曲线穿过标签、邻近点、画布溢出和 GeoGebra 手工偏移。普通情况优先使用 `1`、`1.25`、`1.5` 倍距离；`1.75` 和 `2` 倍仅作为消除无法通过换方向解决的标签互撞的后备位置。
...
当前标签布局还使用 76 个真实 GeoGebra 文件（70 个去重图形）进行批量回归：全部文件均可转换，70/70 个唯一图形均可由 Asymptote 编译；在 625 个可见标签的估算边界框中未发现标签互相重叠，并且只有 3 个标签需要使用超过 `1.5` 倍的后备距离。
